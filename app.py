...
def load_bundled_product_dataset() -> pd.DataFrame | None:
    ...
    dataset_path = Path("sample_product_reviews.csv")
    if dataset_path.exists():
        frames.append(pd.read_csv(dataset_path))
    return pd.concat(frames, ignore_index=True)


def render_header(title: str = "Sentiment Studio", description: str | None = None):
...
def import_speech_recognition_module():
    try:
        ...
        if PROJECT_VENV_SITE_PACKAGES.exists():
            site_packages = str(PROJECT_VENV_SITE_PACKAGES)
            ...
            try:
                ...
                pass
        raise RuntimeError(
            "SpeechRecognition is not available in the current environment. "
            "Run Streamlit from the project `.venv` or install dependencies there."
        ) from first_error
...
def render_try_your_own_text_section(trained_models: dict[str, object], selected_models: list[str]):
    ...
    with centered_mid:
        ...
        with st.container(border=True):
            ...
            st.caption("Emoji-aware preprocessing, vectorization, and model inference are reused exactly from training.")
        ...
        with st.container(border=True):
            ...
            st.caption("Upload a short `.wav` file and reuse one of the trained classical ML models for audio-to-text sentiment analysis.")
            st.info("Use a clear English `.wav` file with short duration for the best transcription result.")
            audio_model = st.selectbox(
                "Choose audio model",
                options=analysis_options,
                key="audio_analysis_model",
            )
            ...
            if st.button("Analyze Audio Sentiment", use_container_width=True, key="audio_analysis_button"):
                if uploaded_audio is None:
                    ...
                    model = trained_models.get(audio_model)
                    if model is None:
                        st.warning("That model is not available from the latest run. Rerun the analysis with it selected.")
                    else:
                        try:
                            ...
                            predicted_label, confidence = predict_with_trained_model(audio_model, model, extracted_text)
                            render_analysis_result(predicted_label, confidence)
...
def render_home_page():
    ...
    with intro_col:
        ...
        st.markdown(
            """
            Sentiment Studio helps teams move from raw text to actionable decisions. Use the app to:
...
def main():
    configure_page("Sentiment Studio")
    apply_global_styles()
    render_home_page()
